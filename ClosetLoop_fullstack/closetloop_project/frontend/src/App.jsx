      ...
      <div className="auth-panel">
        <div className="auth-panel-inner">
          ...
          <div className="auth-heading">
            <span className="eyebrow">{mode === "login" ? "WELCOME BACK" : "JOIN THE LOOP"}</span>
            <h2>{mode === "login" ? "เข้าสู่ ClosetLoop" : "สร้างบัญชีใหม่"}</h2>
            <p>{mode === "login" ? "กลับมาสนุกกับตู้เสื้อผ้าของคุณต่อ" : "เริ่มต้น wardrobe journey ของคุณวันนี้"}</p>
          </div>
          ...
          <form onSubmit={submit} className="form">
            ...
            <button className="btn btn-dark full" disabled={busy}>{busy ? "กำลังดำเนินการ..." : mode === "login" ? "เข้าสู่ระบบ" : "สมัครสมาชิก"} <ArrowRight size={18}/></button>
          </form>
...
function Field({label,value,onChange,type="text",placeholder,required}) {
  return <label className="field"><span>{label}</span><input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder} required={required}/></label>
}
